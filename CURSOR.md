...
本演習ではBasicプランを利用しますが、GPT-4の利用がすぐできなくなりますので、OpenAIから発行したAPIキーを設定することで、GPT-4を利用する方法をとります。APIキーは担当者から発行されたものを入手して下記「5. CursorへのAPIキーの設定」のとおり適用ください。配布したAPIキーは演習後１週間は有効ですが、その後無効化されますのでご了承ください。
